...
const Button: React.FC<ButtonProps> = ({
    ...
    const sizeStyles = {
        ...
        xl: "px-14 py-7 text-base uppercase tracking-[0.5em] font-black",
    };
    ...
    if (variant === "primary" || variant === "outline") {
        return (
            <HoverBorderGradient
                ...
                className={cn(
                    "headline-font font-black rounded-full transition-all flex items-center justify-center gap-3 cursor-pointer select-none",
                    variant === "primary" ? "bg-black text-white" : "bg-transparent text-white border border-white/10",
                    sizeStyles[size]
                )}
                {...(props as any)}
            >
